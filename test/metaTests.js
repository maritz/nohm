...
exports.meta = {
  ...
  version: async (t) => {
    ...
    redis.get(prefix + ':meta:version:UserMetaMockup', function(err, version) {
      ...
      t.same(
        hash.digest('hex'),
        version,
        'Version of the metadata did not match.',
      );
      t.done();
    });
  },
  ...
  'version in instance': async (t) => {
    ...
    redis.hget(prefix + ':hash:UserMetaMockup:1', '__meta_version', function(
      err,
      version,
    ) {
      errLogger(err);
      t.same(
        user.meta.version,
        version,
        'Version of the instance did not match metaData.',
      );
      t.done();
    });
  },
    ...
    test = nohm.model('TestVersionMetaMockup', {
      ...
      metaCallback: function(err, version) {
        ...
        t.same(
          ...
          true,
          'Meta version inDb was not false.',
        );
  ...
  idGenerator: async (t) => {
    ...
    async.parallel(
      [
        function(next) {
          redis.get(prefix + ':meta:idGenerator:UserMetaMockup', function(
            err,
            generator,
          ) {
            errLogger(err);
            t.same(
              user.options.idGenerator.toString(),
              generator,
        ...
        function(next) {
          redis.get(prefix + ':meta:idGenerator:CommentMetaMockup', function(
            err,
            generator,
          ) {
            errLogger(err);
            t.same(
              comment.options.idGenerator.toString(),
              generator,
  ...
  properties: async (t) => {
    ...
    async.parallel(
      [
        function(next) {
          redis.get(prefix + ':meta:properties:UserMetaMockup', function(
            err,
            properties,
          ) {
            errLogger(err);
            t.same(
              JSON.stringify(stringify_functions(user.meta.properties)),
              properties,
        ...
        function(next) {
          redis.get(prefix + ':meta:properties:CommentMetaMockup', function(
            err,
            properties,
          ) {
            errLogger(err);
            t.same(
              JSON.stringify(stringify_functions(comment.meta.properties)),
              properties,
