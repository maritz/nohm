...
/**
 * Preparing populate string/array to work
 */
exports.preparePopulate=function preparePopulate(pop){
  var populateParam;
  if(Array.isArray(pop)){
    populateParam=pop;
  }else{
    populateParam=pop.split(':');
  }
  return populateParam;
};

/**
 * Very simple but useful populate method
 * @param  {string}   pop       format model:populationName or array ['model','populationName']
 * @param  {Function} callback [description]
 * @return {[type]}            [description]
 */
exports.populate=function populate(pop,callback){
  var self=this;
  if(!pop){return callback(new Error('There is nothing to populate.'));}

  var populateParam=self.preparePopulate(pop);

  console.log('populate',populateParam);
  if(populateParam.length!=2){
    return callback(new Error("Wrong populate parameter."));
  }

  self.getAll(populateParam[0],populateParam[1]).then(function(populate_){
    var population=populate_.map(function(pop){
      return orm.factory(populateParam[0]).load(pop).then(function(pop_){
        return pop_;
      });
    });
    Promise.all(population).then(function(population_){
      callback.call(self,null,population_);
    });
  });
...
exports.findAndLoad = function findAndLoad(searches, callback) {
  var self = this;
  var populate=false;
  if(searches.hasOwnProperty('populate')){
    populate=searches.populate;
    delete searches.populate;
  }
      ...
      if (ids.length > 0) {
        async.map(ids, function (id, done) {
          ...
          instance.load(id, function (err) {
            if(populate){
              var populateParam=pop.split(':');
              self.populate(populateParam,function(err,population){
                // when we want population result is model properties rather than model itself
                loadedData[populateParam[1]]=population;
                done(err,loadedData);
              });
            }else{done(err, instance);}
          });
        }, callback);
      }
    }
 ...
 * @param  {array}   data     can be array of results from db/object/undefined if we are removing from model itself
 * @return {array}            [model.props / data] without sensitive data
 */
exports.rmSensitive=function(data){
  
  var original=this.properties;
  var rm=function(obj){
    var keys=Object.keys(obj);
    for(var i=0;i<keys.length;i++){
      if(original.hasOwnProperty(keys[i])){
  ...
  if(data){
    // remove from given data

    if(Array.isArray(data)){
      if(data.length===0)return null;
      _.forEach(data,function(obj_){
        rm(obj_);
      });
      return data;
    }else{
