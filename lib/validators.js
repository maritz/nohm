...
if (typeof(window) !== 'undefined' && typeof(nohmValidationsNamespaceName) !== 'undefined') {
  ...
  for (var i in nohmValidations.extraValidations) {
    for (var name in nohmValidations.extraValidations[i]) {
      if (nohmValidations.extraValidations[i].hasOwnProperty(name)) {
        validators[name] = nohmValidations.extraValidations[i][name];
      }
    }
  }
  ...
  nohm.validate = function (modelName, data) {
    ...
    for (var key in data) {
      if (data.hasOwnProperty(key) && model.hasOwnProperty(key)) {
        errors[key] = validateProperty(key, data[key], model[key]);
      }
    }
