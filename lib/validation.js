 ...
 */
exports.valid = function valid(key, setDirectly, callback) {
  var validbool = true,
  p,
  self = this;
...
exports.__checkUniques = function __checkUniques(setDirectly, saveCallback) {
  ...
  uniqueLocker = function uniqueLocker(propName, callback) {
    if (setDirectly && !self.id) {
      Nohm.logError('Checking for uniques without having an id set. self:' + util.inspect(self));
    }
    ...
    if (setDirectly) {
       ...
       */
      self.getClient().setnx(Nohm.prefix.unique + self.modelName + ':' + propName + ':' + self.p(propName), self.id, checkCallback);
    } else {
      self.getClient().exists(Nohm.prefix.unique + self.modelName + ':' + propName + ':' + self.p(propName), checkCallback);
    }
  };
  for (p in tmp) {
    if (tmp[p].unique === true && tmp[p].value !== '' && (tmp[p].__updated || self.id === null)) {
      i = i + 1;
      finalArgs.push(p + '1');
      initArgs.push(p);
    ...
    finalArgs.push(function () {
      ...
      if (setDirectly && ! success) {
        for (i = 0, len = tmpUniques.length; i < len; i = i + 1) {
          ...
          self.getClient().del(tmpUniques[i], self.logError);
        }
      }
