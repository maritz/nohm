...
var ModelClass = {
  ...
  __checkUniques: function __checkUniques(saveCallback) {
    ...
    uniqueLocker = function uniqueLocker(propName, callback) {
      ...
      redis.setnx(uniqueprefix + self.modelName + ':' + propName + ':' + self.p(propName), self.id, function (err, value) {
        /*
  ...
  __link: function __link(obj, name, cb) {
    ...
    if (!this.id) {
      callback('save parent object before adding a child.');
    } else if (!obj.id) {
      obj.save(addition);
    } else {
      addition();
  ...
  __unlink: function __unlink(obj, name, cb) {
    ...
    if (!this.id) {
      callback('save parent object before removing a child.');
    } else if (!obj.id) {
      obj.save(removal);
    } else {
      removal();
  ...
  link: function link(obj, name, directChange) {
    ...
    if (this.id && directChange && obj.valid()) {
      console.dir('adding a relation immediately');
      this.__link(obj, name, callback);
    } else {
      console.dir('adding a relation addition to the queue');
  ...
  unlink: function unlink(obj, name, directChange) {
    ...
    if (this.id && directChange && obj.id) {
      console.dir('removing a relationship instantly.');
       ...
       */
      console.dir('trying to remove one of the relationchanges');
      for (var change in this.relationChanges) {
        if (this.relationChanges.hasOwnProperty(change)
          ...
          console.dir('taking a relationchange from the queue. success!');
          delete this.relationChanges[change];
        }
      }
    } else {
      console.dir('adding a removal to the queue');
      this.relationChanges.push({
        action: 'remove',
        name: name,
        callback: callback,
