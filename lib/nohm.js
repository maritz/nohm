...
Nohm.logError = function logError(err) {
  if (err) {
    //it is always better to throw errors
    throw new Err(err);
  }
};
...
var __updateMeta = function (callback) {
  ...
  var version_key = Nohm.prefix.meta.version + this.modelName;
...
moduleNames.forEach(function (name) {
  ...
  // promisify basic functions
  var keys=Object.keys(modules[name]);
  var toPromisify=['find','getAll','load','findAndLoad','sort'];
  for(var i=0;i<keys.length;i++){
    var key=keys[i];
    if(toPromisify.indexOf(key)>-1){
      modules[name][key]=Promise.promisify(modules[name][key]);
    }
  }
  // end of promisification
