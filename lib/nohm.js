    ...
    for (var p in tmp) {
      if (tmp[p].unique === true && (tmp[p].__updated || self.id === null)) {
        ...
        functions[p] = ['_' + i, function (propName, callback) {
          redis.setnx(prefix + ':uniques:' + self.modelName + ':' + propName + ':' + self.p(propName), (self.id || true), function (err, value) {
            /*
        ...
        for (var p in props) {
          ...
          if (props[p].unique === true && props[p].__updated) {
            if (self.__inDB) {
              ...
              // since we didn't have the id when checking the uniqueness we add it now.
              redis.set(prefix + ':uniques:' + self.modelName + ':' + p + ':' + props[p].value, self.id);
            }
          }
          if (props[p].index === true && props[p].__updated) {
            if (self.__inDB) {
              redis.srem(prefix + ':index:' + self.modelName + ':' + p + ':' + props[p].__oldValue);
            }
            redis.sadd(prefix + ':index:' + self.modelName + ':' + p + ':' + props[p].value, self.id, function (err) {
              sys.debug('Nohm: Adding an index resulted in a redis error: ' + sys.inspect(err));
              // TODO: add some kind of logging for errors.
            })
          }
          self.__inDB = true;
