...
var ModelClass = {
  ...
  load: function load(id, callback) {
    ...
    id = parseInt(id, 10);
    if (isNaN(id)) {
      this.logError('Trying to pass load() a wrong kind of id. Needs to be a number. (still trying to load it though)');
    }
  ...
  remove: function remove(callback) {
    ...
    // redis KEYS is slow. maybe there's a better solution? (SLOW AS FUCK, THINK GODDAMNIT!)
