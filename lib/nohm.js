  ...
  __create: function __create(callback) {
    ...
    client.incr(prefix + ':ids:' + this.modelName, function (err, newId) {
      if (!err) {
        ...
        sys.debug('Nohm: Creating an object resulted in a client error: ' + sys.inspect(err));
        if (typeof callback === 'function') {
          callback(err);
        } else {
          throw err;
  ...
  __update: function __update(all, callback) {
    ...
    realUpdate = function realUpdate(err) {
      ...
      if (typeof callback !== 'function' && err) {
        sys.debug('Nohm: Updating an object resulted in a client error: ' + sys.inspect(err));
        throw err;
      } else if (err) {
        callback(err);
      } else {
