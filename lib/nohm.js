...
var ModelClass = {
  ...
  remove: function remove(callback) {
    var self = this,
    expectQueued = function (err, value) {
      if (value.toString() !== 'QUEUED') {
        self.logError('Queing of multi/exec failed. returned value: ' + value.toString());
      }
    };
    if (!this.id) {
    ...
    redis.keys(relationsprefix + this.modelName + ':' + '*:' + this.id,
               function (err, keys) {
                  ...
                  inConductCallback = function (key, callback) {
                    ...
                    redis.smembers(key, function (err, value) {
                      ...
                      sremFunc = function (key) {
                        return function () {
                          redis.srem(key, self.id, expectQueued);
                        };
                      };
                ...
                conductorsDone.push(function () {
                  ...
                  redis.del(hashprefix + self.modelName + ':' + self.id, expectQueued);
