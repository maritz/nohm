...
var numToAlpha = exports.numToAlpha = function numToAlpha(num) {
  num = parseInt(num, 10);
...
var ModelClass = {
  ...
  load: function load(id, callback) {
    ...
    redis.hgetall(this.getHashKey(id), function (err, values) {
      var p, value;
      if (err) {
  ...
  find: function find(searches, callback) {
    ...
    if (sets.length === 0 && zsetKeys.length === 0) {
      redis.smembers(prefix + ':idsets:' + this.modelName, function (err, ids) {
  ...
  __create: function __create(callback) {
    ...
    redis.incr(prefix + ':ids:' + this.modelName, function (err, newId) {
      if (!err) {
        redis.sadd(prefix + ':idsets:' + self.modelName, newId, function (err) {
  ...
  __realDelete: function __realDelete(callback) {
    ...
    // 1) add all relationkeys to a set when creating them, fetch those instead of redis.KEYS
    redis.keys(relationsprefix + this.modelName + ':*:' + self.id,
             function (err, keys) {
              ...
              conductorsDone.push(function () {
                ...
                redis.srem(prefix + ':idsets:' + self.modelName, self.id, self.__expectQueued);
                ...
                for (p in self.properties) {
                  if (self.properties.hasOwnProperty(p)) {
                    ...
                    if (self.properties[p].__numericIndex) {
                      redis.zrem(scoredindexprefix + self.modelName + ':' + p,
                                 self.id,
                                 self.__expectQueued);
                    }
                  }
  ...
  getAll: function getAll(objName, name) {
    ...
    redis.smembers(this.relationKey(objName, name),
                    function (err, value) {
                      if (err) {
                        self.logError(err);
                      }
  ...
  numLinks: function numLinks(objName, name) {
    ...
    redis.scard(relationsprefix + this.modelName + ':' + name + ':' +
                    objName + ':' + this.id,
                    function (err, value) {
                      if (err) {
                        self.logError(err);
  ...
  __checkUniques: function __checkUniques(setDirectly, saveCallback) {
    ...
    uniqueLocker = function uniqueLocker(propName, callback) {
      ...
      var checkCallback = function (err, value) {
        ...
        if (!setDirectly) {
          // redis.exists returns 1 if the value exists, redis.setnx returns 1 if the value did not exist.
      ...
      finalArgs.push(function () {
        ...
        if (setDirectly && ! success) {
          for (i = 0, len = tmpUniques.length; i < len; i = i + 1) {
            ...
            redis.del(tmpUniques[i], self.logError);
          }
        }
