  ...
  var _save = function (id) {
    ...
    self.valid(false, true, function (valid) {
      if (!valid && typeof callback === 'function') {
        ...
        __create.call(self, callback, silent);
      } else if (valid) {
        __update.call(self, false, callback, false, silent);
      }
    });
  };
 ...
 * @ignore
 */
var __create = function __create(callback, silent) {
  var self = this;
  __generate_id.call(this, function (newId) {
    self.getClient().sadd(Nohm.prefix.idsets + self.modelName, newId, function (err) {
      ...
      self.__setUniqueIds(newId, function (err) {
        if (err) { Nohm.logError(err); }
        self.id = newId;
        __update.call(self, true, callback, true, silent);
      });
    });
  });
};
 ...
 * @ignore
 */
var __update = function __update(all, callback, isCreation, silent) {
  var hmsetArgs = [],
      ...
      async.forEachSeries(self.relationChanges,
        ...
        function (err) {
          if (typeof callback !== 'function' && err) {
            ...
            if (!silent) {
              Nohm.fire(isCreation ? 'create' : 'update', self);
              Nohm.fire('change', self, diff);
            }
