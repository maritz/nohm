...
var validationsFlatten = function (model, exceptions) {
  model = new model();
 ...
 *          },*/
  ...
  Object.keys(exceptions).forEach(function (key) {
    var value = exceptions[key];
    if (Array.isArray(value)) {
      exceptionObject[key] = value; // value should be like [true, false, true]
    }
    exceptionStrings.push(key);
  });
  
  Object.keys(props).forEach(function (key) {
    var isExcepted = exceptionStrings.indexOf(key) !== -1 && ! exceptionObject.hasOwnProperty(key);
    if ( ! isExcepted) {
      ...
      if (Array.isArray(vals) && vals.length > 0) {
        ...
        vals.forEach(function (val, index) {
          if ( ! exceptionObject[key] || exceptionObject[key][index]) {
 ...
 *    - `exceptions`  - Object containing exceptions for the validations export - see example for details
...
function connect(options){
  ...
  Object.keys(models).forEach(function (name) {
    ...
    if (exceptions.hasOwnProperty(name) && ! Array.isArray(exceptions[name]) ) {
      return; // exception set, but no fields
    }
    arr.push(validationsFlatten(model, exceptions[name]));
  });
